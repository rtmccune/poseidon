...
def test_filter_by_eastern_time_success(tmp_path):
    ...
    et_hours = pd.to_datetime(result_df['start_time_ET'], utc=True).dt.tz_convert('America/New_York').dt.hour
    assert all(h >= 6 for h in et_hours)
    assert all(h < 20 for h in et_hours)
...
def test_filter_by_eastern_time_missing_column(tmp_path, capsys):
    ...
    captured = capsys.readouterr()
    assert "ERROR: Input CSV must contain the column 'start_time_UTC'" in captured.err
...
def image_setup(tmp_path):
    ...
    (image_folder / "CAM_VA_10_20230202113000.jpg").touch() # Event 2 & 3 (overlap)
    ...
    return image_folder, csv_file, destination_folder, subfolder_name, padding_hours
...
def test_organize_images_success_and_overlap(image_setup, capsys):
    ...
    img1_path = dest_folder / folder1 / subfolder / "CAM_NC_01_20230101123000.jpg"
    img2_path = dest_folder / folder1 / subfolder / "CAM_NC_01_20230101150000.jpg"
    ...
    img3_path = dest_folder / folder2 / subfolder / "CAM_VA_10_20230202103000.jpg"
    img4_path = dest_folder / folder2 / subfolder / "CAM_VA_10_20230202113000.jpg"
    ...
    img5_path = dest_folder / folder3 / subfolder / "CAM_VA_10_20230202103000.jpg" # Overlap
    img6_path = dest_folder / folder3 / subfolder / "CAM_VA_10_20230202113000.jpg" # Overlap
    ...
    assert not (dest_folder / folder1 / subfolder / "CAM_NC_01_20230101085900.jpg").exists()
...
def test_organize_images_folder_not_found(tmp_path, capsys):
    ...
    pd.DataFrame(columns=[
        'sensor_ID', 'start_time_UTC', 'end_time_UTC'
