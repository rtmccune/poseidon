...
for poly_name, files in polygon_files.items():
    ...
    for metric in metrics:
        ...
        if sensor_df_clean is not None and 'SensorWSE_m' in sensor_df_clean.columns:
            # We need to load the companion WSE csv generated by roadway_analyzer
            wse_csv_path = f[0].replace('_accessibility_time_series.csv', '_wse_time_series.csv')
            
            if os.path.exists(wse_csv_path):
                wse_df = pd.read_csv(wse_csv_path)
                wse_df['datetime_utc'] = pd.to_datetime(wse_df['Time'], errors='coerce', utc=True)
                
                # Merge WSE data with our existing combined_df
                wse_merged = pd.merge(combined_df, wse_df[['datetime_utc', 'MeanWSE', 'MaxWSE', 'MedianWSE']], 
                                      on='datetime_utc', how='inner')
                
                # Setup the equivalent WSE metric name
                wse_metric = metric.replace('Depth', 'WSE')
                
                if wse_metric in wse_merged.columns:
                    plot7_df = wse_merged[wse_merged['PointColor'] == 'green'].dropna(subset=[wse_metric, 'SensorWSE_m', 'Coverage']).copy()
