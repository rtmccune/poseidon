...
def CIRNangles2R(azimuth, tilt, swing):
    ...
    R[0, 0] = -np.cos(azimuth) * np.cos(swing) - np.sin(azimuth) * np.cos(
        tilt
    ) * np.sin(swing)
    R[0, 1] = np.cos(swing) * np.sin(azimuth) - np.sin(swing) * np.cos(
        tilt
    ) * np.cos(azimuth)
    R[0, 2] = -np.sin(swing) * np.sin(tilt)
    R[1, 0] = -np.sin(swing) * np.cos(azimuth) + np.cos(swing) * np.cos(
        tilt
    ) * np.sin(azimuth)
    R[1, 1] = np.sin(swing) * np.sin(azimuth) + np.cos(swing) * np.cos(
        tilt
    ) * np.cos(azimuth)
...
def mergeRectifyFolder(
    folder_path, intrinsics, extrinsics, grid_x, grid_y, grid_z
):
...
def mergeRectifyLabels(
    image_path, intrinsics, extrinsics, grid_x, grid_y, grid_z
):
...
def plot_depth_maps(
    zarr_store_path, grid_z, plotting_folder, depth_min=None, depth_max=None
):
    ...
    pattern = re.compile(
        r"(?P<cam_id>CAM_[A-Z]{2}_\d{2})_(?P<timestamp>\d{14})_rectified"
    )
    ...
    for array_name in store.keys():
        ...
        if match:
            ...
            grayscale_image = np.dot(
                rectified_image[..., :3], [0.2989, 0.5870, 0.1140]
            )
            ...
            max_elev_point_indices = np.unravel_index(
                np.nanargmin(depth_map), depth_map.shape
            )

            # Plot the image
            plt.imshow(
                grayscale_image, cmap="gray"
            )  # Assuming ir_array is your grayscale image
            ...
            plt.scatter(
                max_elev_point_indices[1],
                max_elev_point_indices[0],
                c="red",
                s=10,
                marker="o",
            )
            ...
            plt.savefig(
                os.path.join(
                    plotting_folder,
                    f"{cam_id}_{timestamp}_depth_map_rectification.png",
                ),
                bbox_inches="tight",
                pad_inches=0.1,
                dpi=300,
