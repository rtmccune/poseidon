...
class TestGridGenerator:
    ...
    def test_create_point_array_meters_no_conversion(self):
        ...
        gen = GridGenerator(
            'fake.las',
            min_x_extent=0, max_x_extent=500,
            min_y_extent=0, max_y_extent=500,
            extent_units='meters',
            lidar_units='meters'
        )
    ...
    def test_gen_grid_flat_z(self, mock_os_makedirs, mock_zarr_open):
        ...
        gen = GridGenerator(
            'fake.las',
            min_x_extent=10, max_x_extent=30,
            min_y_extent=10, max_y_extent=30,
            extent_units='meters', lidar_units='meters'
        )
        ...
        expected_calls = [
            call(os.path.join("test/output", "flat_grid_x_10m.zarr"), 
                 mode='w', shape=grid_x.shape, dtype=grid_x.dtype, chunks=True),
            call(os.path.join("test/output", "flat_grid_y_10m.zarr"), 
                 mode='w', shape=grid_y.shape, dtype=grid_y.dtype, chunks=True),
            call(os.path.join("test/output", "flat_grid_z_10m.zarr"), 
                 mode='w', shape=grid_z.shape, dtype=grid_z.dtype, chunks=True)
        ]
    ...
    def test_gen_grid_interpolated_z(self):
        ...
        gen = GridGenerator(
            'fake.las',
            min_x_extent=10, max_x_extent=20,
            min_y_extent=10, max_y_extent=20,
            extent_units='meters', lidar_units='meters'
        )
    ...
    def test_gen_grid_bad_z_type(self):
        ...
        gen = GridGenerator(
            'fake.las',
            min_x_extent=0, max_x_extent=10,
            min_y_extent=0, max_y_extent=10,
            extent_units='meters',
            lidar_units='meters'
        )
